use std::{fmt, collections::HashMap, io::{self, stdin, stdout, Write}, iter::Peekable, env, fs, str::Chars};
use Expr::*;
mod lexer;
use lexer::*;
mod macros;
            ...
            Sym => {
                if let Some(_) = lexer.next_if(|t| t.kind == OpenParen) {
                    ...
                    if let Some(_) = lexer.next_if(|t| t.kind == CloseParen) {
                        return Ok(Fun(Box::new(Self::var_or_sym_from_name(&name.text)), args))                        
                    }
                    ...
                    if close_paren.kind == CloseParen {
                        Ok(Fun(Box::new(Self::var_or_sym_from_name(&name.text)), args))
                    } else {
                    ...
                    match token.kind {
                        TokenKind::Sym => {
                            if let Some(rule) = self.rules.get(&token.text) {
                                ...
                                println!(" => {}", &new_expr);
                                self.shaping_history.push(self.current_expr.replace(new_expr).expect("current_epr must have something"));
                            } else {
