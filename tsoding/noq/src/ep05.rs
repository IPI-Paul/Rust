use std::{fmt, collections::HashMap, io::{stdin, stdout, Write, self}, env, fs};
use Expr::*;
mod lexer;
use lexer::*;
    ...
    match err {
        ...
        Error::Runtime(err) => match err {
            ...
            RuntimeError::AlreadyShaping(_loc) => {
                eprintln!("ERROR: already shaping an expression. Finish the current shaping with {} first.", TokenKind::CloseBrace);
            }
            ...
            RuntimeError::UnknownStartegy(name, _loc) => {
                eprintln!("ERROR: unknown rule application trategy '{}'", name);
            }
        ...
        if let Err(err) = parse_and_process_command(&mut context, &mut lexer) {
            match err {
                ...
                Error::Runtime(RuntimeError::AlreadyShaping(loc)) => {
                    eprintln!("{}: ERROR: Already shaping an expression. Finish the current shaping with {} first.", loc, TokenKind::CloseBrace);
                }                           
