...
impl fmt::Display for TokenKind {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    match self {
      ...
      OpenBrace => write!(f, "close brace"),
      CloseBrace => write!(f, "close brace"),
      Bar => write!(f, "bar"),
      Bang => write!(f, "bang"),
      Comma => write!(f, "comma"),
...
impl<Chars: Iterator<Item=char>> Lexer<Chars> {
    ...
    fn chop_tokens_from_chars(&mut self) -> Token {
      assert!(!self.exhausted, "Completely exhausted lexer. The lexer Must Always end with the terminators. If the lexer caller tries to pull tokens after terminators, this is a bug.");
      ...
      match self.chars.next() {
          Some(x) => {
            ...
            match x {
              ...
              '{' =>  Token {kind: TokenKind::OpenBrace, text, loc},
              '}' =>  Token {kind: TokenKind::CloseBrace, text, loc},
