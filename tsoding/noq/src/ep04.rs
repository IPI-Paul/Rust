use std::{fmt, collections::HashMap, io::{stdin, stdout, Write}, env, fs};
use Expr::*;
mod lexer;
use lexer::*;
mod macros;
...
impl Expr {
    ...
    fn parse(lexer: &mut Lexer<impl Iterator<Item=char>>) -> Result<Self, Error> {
      ...
      fn apply_impl(rule: &Rule, expr: &Expr, strategy: &mut impl Strategy) -> (Expr, bool) {
        if let Some(bindings) = pattern_match(&rule.head, expr) {
          ...
          let new_expr = match resolution.action {
              Action::Apply => substitue_bindings(&bindings, &rule.body),
...
fn substitue_bindings(bindings: &Bindings, expr: &Expr) -> Expr {
    match expr {
        ...
        Op(op, lhs, rhs) => {
            Op(
                *op, 
                Box::new(substitue_bindings(bindings, lhs)), 
                Box::new(substitue_bindings(bindings, rhs))
            )
        },
        Fun(head, args) => {
            let new_head = substitue_bindings(bindings, head);
...
impl Context {
    fn parse_applied_rule(&self, lexer: &mut Lexer<impl Iterator<Item=char>>) -> Result<Rule, Error> {
        ...
        match token.kind {
            ...
            _ => Err(Error::UnexpectedToken(TokenKindSet::single(TokenKind::Reverse).set(TokenKind::Rule), token))
        }
    }
                ...
                if let Some(expr) = &self.current_expr {
                  ...
                  let new_expr = match &strategy_name.text as &str {
                      ...
                      x => match x.parse() {
                          ...
                          _ => return Err(Error::UnknownStartegy(strategy_name.text, strategy_name.loc))
                      }
                  };
                  println!(" => {}", &new_expr);
      ...
      while !context.quit && lexer.peek_token().kind != TokenKind::End {
        if let Err(err) = context.process_command(&mut lexer) {
            match err {
              ...
              Error::UnknownStartegy(name, loc) => {
        ...
        while !context.quit {
            ...
            if lexer.peek_token().kind != TokenKind::End {
              ...
              match result {
                  ...
                  Err(Error::UnknownStartegy(name, loc)) => {
                      eprint_repl_loc_cursor(prompt, &loc);
