            ...
            Rule::Replace => {
                if let Some(bindings) = expr!(apply_rule(Strategy, Head, Body, Expr)).pattern_match(expr) {
                    *match_count += 1;
                    ...
                    let meta_strategy = bindings.get("Startegy").expect("Varaible `Strategy` is present in the meta pattern");
    ...
    match err {
        ...
        Error::Runtime(err) => match err {
            ...
            RuntimeError::AlreadyShaping(_loc) => {
                eprintln!("ERROR: a;ready shaping an expression. Finish the current shaping with {} first.", TokenKind::Done);
            }
