...
fn get_file_path_with_extension(model: &Model, request: &Request) -> String {
  let mut url = format!(".{url}", url = request.url().to_string());
  let urls = model.tfpd.iter()
    ...
    match (request.method(), request.url()) {
        ...
        _ => {
          if request.url().contains("/docs.gl/") {
            let url = get_file_path_with_extension(&model, &request);
