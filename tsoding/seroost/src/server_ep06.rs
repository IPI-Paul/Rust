...
fn serve_api_search(model: Arc<Mutex<Model>>, mut request: Request) -> io::Result<()> {
  ...
  let result = model.search_query(&body, false);
...
fn get_file_path_with_extension(model: Arc<Mutex<Model>>, request: &mut Request) -> String {
  ...
  url = decode(&url).expect("UTF-8").to_string();
  let mut buf = Vec::new();
  if let Err(err) = request.as_reader().read_to_end(&mut buf) {
    eprintln!("ERROR: could not read the body of the request: {err}");
  }
  ...
  let model = model.lock().unwrap();
  let result = model.search_query(&body, true);
...
fn serve_request(model: Arc<Mutex<Model>>, request: Request) -> io::Result<()> {
    ...
    match (request.method(), request.url()) {
        ...
        _ => {
          ...
          } else if request.url().contains("/run/media/") || request.url().contains("src/") {
            let content_type = if request.url().contains(".pdf") { 
              "application/pdf; charset=utf-8" 
            } else if request.url().contains(".json") { 
              "application/json; charset=utf-8" 
            } else if request.url().contains(".js") {
              "text/javascript; charset=utf-8" 
            } else if request.url().contains(".css") {
              "text/css; charset=utf-8" 
            // } else if request.url().contains(".md") {
            //   "text/markdown; charset=utf-8" 
            } else if request.url().contains(".csv") | request.url().contains(".txt") {
              "text/plain; charset=utf-8" 
            } else {
              "text/html; charset=utf-8"
            };
            let url = request.url().to_string().replace("%20", " ");
            let mut url = decode(&url).expect("UTF-8").to_string();
            if request.url().starts_with("/src/") {
              url = ".".to_owned() + &url;
            }
            serve_static_file(request, &url, content_type)
          } else {
            serve_404(request)
          }
        },
